...
| **ADR-F039** | CSP source lists are **constructed programmatically with host-sources emitted first**, never hand-written. WebKit (GTK 2.50, confirmed) stops matching a host-source when a keyword or scheme-source precedes it in the same directive — `img-src 'self' <origin>` blocks where `img-src <origin> 'self'` loads. Ordering is a correctness requirement, so it must not be expressible incorrectly. | Accepted |
| **ADR-F040** | CSP behaviour is **validated per engine in the conformance suite**, never reasoned about from the specification. The `img-src` bug is spec-illegal — a source list is a permissive union — and no amount of reading the standard would have predicted it. | Accepted |
...
| **R24** | The `img-src` finding tested **over-blocking** (a permissive source failing to permit). The security-relevant direction is **under-blocking** — whether any ordering causes a restrictive directive to silently fail open. `default-src 'none'` is the backstop for the entire content sandbox (ADR-F005), and its failure mode has not been probed. | **High** | Unknown | Extend the F1c probe: assert that each restrictive directive still *denies* under every source-ordering permutation, not merely that permissive ones allow. A fail-open ordering is an R2-class finding, not a rendering bug. |
| **R25** | Host-first ordering (ADR-F039) is a workaround for an engine bug, not a fix. A future WebKit release that corrects the union semantics, or a different WebKit port that never had the bug, could change behaviour under the same CSP string. | Medium | Medium | Programmatic construction (ADR-F039) plus a conformance assertion that fails loudly if ordering stops mattering, so the workaround's expiry is detected rather than assumed. Do not assume WKWebView shares WebKitGTK's behaviour — this reinforces R23 rather than being independent of it. |
