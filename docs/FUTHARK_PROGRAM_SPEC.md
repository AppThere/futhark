...
| **ADR-F050** | F2b enumerates against a **fixed catalogue of infoset and container features**, and every feature resolves to one of three states: `Observed`, `CheckedAndAbsent`, or `NotCovered`. An open-ended "what did we find" list cannot distinguish a construct the corpus lacked from one the harness never looked for; a checklist can. This is the `Class::Unclassified` and ADR-F042 shape applied to a deliverable that has no verdict to carry it. | Accepted |
