...
| **F2b** | Real-corpus characterisation | Not "does rbook fail" — that is settled and rbook is no longer the subject. **What infoset and container features do real EPUBs actually contain that `futhark-epub` must preserve?** | Divergence-class distribution and producer distribution over Tier B. The deliverable is a requirements list for the bespoke build, not a verdict. Confirms the ADR-F011 reversal as a side effect. |
