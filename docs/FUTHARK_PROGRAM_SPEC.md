...
| **ADR-F038** | For round-trip spikes the **published artifact is the result manifest**, not the books: content hash, producer string, divergence class, and diff summary per file. This is what makes Tier B usable without redistributing a single copyrighted byte. | Accepted |
...
| ~~**D7**~~ | ~~Corpus sourcing and licensing.~~ **Resolved: two-tier** (ADR-F037/F038). | Note that F1 and F2 have *opposite* corpus needs. F1 wanted synthetic content under full control. F2 wants in-the-wild producer diversity — InDesign, Sigil, calibre, ancient EPUB 2, publisher toolchains — which is exactly what a generated corpus cannot supply and what Tier A's single-toolchain sources (Gutenberg's ebookmaker, Standard Ebooks) under-represent. Tier B carries F2. |
| **D12** | Does Tier B need a minimum producer-diversity bar before F2's result is meaningful? | 200 files from four producers is a weaker test than 60 from twenty. Suggest recording the producer string distribution in the manifest and treating coverage, not count, as the gate. |
