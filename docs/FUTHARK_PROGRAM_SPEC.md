...
| **R26** | **Chromium's leniency masks opaque-origin CSP errors during development.** Chromium permits `'self'` in an opaque-origin frame where WebKit correctly matches nothing. Primary development is on Linux and Windows, so a CSP mistake of this class passes locally and fails on macOS and iOS — the platforms with the least test coverage (R21, R23). | Medium | Medium | Treat the strict engine as the reference, not the convenient one. WebKitGTK runs in CI on every sandbox change, and any CSP assertion that passes on Chromium but not WebKit is a bug in the policy until proven otherwise. |
