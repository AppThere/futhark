...
| **ADR-F011** | ~~EPUB reader/writer starts from `rbook`.~~ **REVERSED by Spike F2.** `futhark-epub` is bespoke over `quick-xml` + `zip`. rbook parses to a model and serialises from it, so the original bytes are gone before a writer exists — structural, not patchable, and no wrapper recovers them. It also stamps `dc:date`/`dcterms:modified` from the current clock, making two consecutive saves of an untouched file differ from each other. Correct behaviour for a builder library; incompatible with ADR-F012. **Evidence base: 12 synthetic containers, zero real books.** The mechanism is architectural and the conclusion is not seriously in doubt, but the spike author explicitly declined to resolve this and the document closed it anyway — recorded here so the gap is visible rather than inherited. Confirmation run: F2b. | **Reversed — on synthetic evidence** |
...
| **ADR-F053** | **Every variant of an enum encoding an epistemic or safety distinction carries a reachability witness**: a test that reaches it through the real code path on realistic input, not by direct construction. Encoding a distinction as a type is a claim about reachability that nothing otherwise verifies — `or_insert` left `FeatureState` with three declared states and two reachable ones, green throughout. An unreachable variant is documentation wearing a type's clothes, and inherits every weakness this project has been encoding *away* from documentation. **A witness is a floor on the type, not a guarantee the distinction is live** — it proves a variant can be produced, never that it is produced for its stated reason. | Accepted |
| **ADR-F055** | **Every detector carries a paired true negative**, not only a true positive: an input differing from the positive case *only* in the construct under test, asserted to produce a different state. This is the differential the repository self-test already applies to checks — must pass clean, must fail dirty — extended to the layer where it was actually needed. Both false-`Observed` bugs (`xmlns:container` matching the mandatory OCF declaration, whitespace-split prose scoring as non-alphabetical attribute order) reached the right variant for the wrong reason, which ADR-F053 cannot see and a true negative catches directly. | Accepted |
| **ADR-F056** | Variants without a witness are sorted into three kinds and treated differently: **unwitnessable by nature** (`unclassified` — a fixture for it would be a fixture for the classifier's blind spot), **unwitnessable without fabrication** (`WriteFailed`, `Panicked` — an invented input witnesses the invention), and **dead** (`Tier::Fixture` — declared, never constructed, and read by consumers as a promise that such records exist). A fourth kind is **beyond the harness** (`timestamp`, `compression-level` — witnessable in principle, but the fixture writer pins them as its control variable, so closing the gap means giving up the control). The first, second, and fourth are documented; **the third is deleted.** Recording them all as "no witness" preserves the one that should not exist. | Accepted |
| **ADR-F060** | **A check with no inputs reports `NotApplicable`, never `ok`.** `check-fuzz-seeds` over zero fuzz targets is `manifest-mismatch: 0` wearing a green tick — the whole point of building it before the first codec crate is defeated if its passing state is indistinguishable from having nothing to check. It therefore ships **with a fixture fuzz target and a known seed**, so the check has a live subject from the day it lands and `check-self-test` has a negative case to perturb. | Accepted |
| **ADR-F061** | **`BeyondTheHarness` is contingent, not permanent.** Unlike the other ADR-F056 kinds it is a property of the *synthetic fixture builder*, not of the variant or of the instrument — real corpora carry varying timestamps and compression levels freely. It is therefore re-checked after the first Tier B run, and **a still-zero `timestamp` count over a real library is a finding about the detector, not about the corpus.** | Accepted |
| **ADR-F062** | Test and harness infrastructure is held to the standard applied to the code it measures. The `read_bytes` race was latent from creation, invisible at two callers, and surfaced only by a third — and it failed loudly. The same defect producing a *truncated but plausible* read would have classified into a real class and been believed. Shared-resource helpers isolate by construction, not by convention. | Accepted |
| **ADR-F058** | **A reported zero is a measurement only where something can produce a non-zero.** Every zero-valued count a summary, report, or dashboard emits requires a demonstrated live path to a non-zero value; absent one, the zero is produced by the missing code rather than by the data. `Class::ManifestMismatch` was frozen in the taxonomy, collected inputs, had no detector, and contributed a confident `manifest-mismatch: 0` to every histogram ever printed. This is stricter than ADR-F053: a variant needs to be *reachable*, a reported count needs to be *reached* by a test that observes it non-zero. | Accepted |
| **ADR-F059** | **Freezing a taxonomy certifies the vocabulary, not the implementation.** ADR-F041 froze the class list to stop the instrument being fitted to its own data, and that remains correct — but a frozen list carries no claim that each class is measured, and reads as though it does. Every class in a frozen taxonomy must therefore have a detector *and* a witness, asserted structurally rather than inferred from a passing fixture run. | Accepted |
| **ADR-F057** | **`NotYetWritten` is coverage debt, not a kind in ADR-F056.** Every ADR-F056 kind states a permanent property of the variant or the harness; an unwritten fixture states only a schedule. Listing them together makes a backlog item look like a classification, and invites the failure this project keeps finding: a taxonomy that reads as complete because an item migrated rather than because work happened. **Invariant: an entry may enter `NotYetWritten` only when its class is newly declared, and may never enter by migration from another kind.** Leaving is by writing the fixture. | Accepted |
| **ADR-F054** | **Every repository check carries a negative test** — an input that makes it fail, run in CI beside the passing case. A check never observed to fail is indistinguishable from a check that stopped looking. This is ADR-F042's liveness requirement applied to the tooling rather than to the probes. | Accepted |
| **ADR-F052** | F2b enumerates against a **fixed catalogue of infoset and container features**, and every feature resolves to one of three states: `Observed`, `CheckedAndAbsent`, or `NotCovered`. An open-ended "what did we find" list cannot distinguish a construct the corpus lacked from one the harness never looked for; a checklist can. This is the `Class::Unclassified` and ADR-F042 shape applied to a deliverable that has no verdict to carry it. **Renumbered from ADR-F050 at 0.13.1** — collided with the producer-family rule, which is older and keeps the number. Sweep citations in `catalogue.rs` and the F2b findings. | Accepted |
| **ADR-F051** | F2b's output type is a **floor, not a requirements list**, and is named so. It cannot be consumed as a complete specification for `futhark-epub` without an explicit widening step that records which corpus produced it and whether that corpus was normalized. The floor-not-ceiling property is structural, not a caveat in a findings document — prose caveats have already been shown to separate from their claims in this project (ADR-F011, §6 of the F2 findings). | Accepted |
| **ADR-F048** | `futhark-epub` **retains the original bytes of every archive entry alongside the parsed model**. The model is a *view*; the retained bytes are the source of truth for serialisation. An entry the editor did not touch is written back verbatim, never re-serialised from the model. This is the specific property rbook's architecture cannot provide, and it is the design requirement F2 exists to have established. | Accepted |
| **ADR-F049** | Metadata that a specification requires to change on modification — `dcterms:modified` above all — is updated **only when the package is actually modified**, never on open or on save-without-edit. Save idempotence is a test: saving an untouched book twice produces identical bytes both times. | Accepted |
...
| **ADR-F026** | Untrusted-input parsing (Codecs) is fuzzed in CI. Every codec crate gets a `cargo-fuzz` target from the day it is created, not retrofitted. **Each target ships with a seeded known-crash input proving the harness reaches the parser** (ADR-F058) — otherwise "0 crashes in 24h" is the `manifest-mismatch: 0` bug at a larger scale and a longer timescale. | Accepted |
...
| **ADR-F050** | D12's coverage gate counts **producer families, not producer strings**. "InDesign 17" and "InDesign 19" are one family; twenty strings can be three families. A raw string count is the R27 shape one level up — a healthy-looking number produced by the wrong mechanism. The verdict **names the families it counted** so a reader can judge coverage instead of trusting a threshold. | Accepted |
...
| **R5** | ~~`rbook` cannot round-trip losslessly.~~ **Realised.** F2 returned 0 of 12 byte-lossless; the cause is architectural. `futhark-epub` is a bespoke build (ADR-F011 reversed, ADR-F048). | — | Confirmed | Cost is now in the plan rather than in the risk register. Phase 1 absorbs it: the read path was always bespoke work, and byte retention is a constraint on its design rather than an additional stage. |
...
| **F2b** | Real-corpus characterisation | Not "does rbook fail" — that is settled and rbook is no longer the subject. **What infoset and container features do real EPUBs actually contain that `futhark-epub` must preserve?** | Three-state feature catalogue per ADR-F052, plus divergence-class and producer distributions over Tier B. Output is a floor (ADR-F051), never a specification. A normalized corpus strips exactly the constructs being enumerated — comments, PIs, exotic namespaces — so a thin result is evidence about the corpus before it is evidence about the ecosystem. Confirms the ADR-F011 reversal as a side effect. |
...
| **D15** | The `PROVISIONAL_PRODUCER_FLOOR` value. | Placeholder is 5, explicitly not-a-decision. Recommendation: set it against **families** (ADR-F050), where the wild population is roughly ten to twelve — InDesign, Sigil, calibre, ebookmaker, Standard Ebooks, Vellum, Pressbooks, Apple Pages, publisher in-house, legacy 2008–2012 converters. A floor of 8 families is defensible; 5 strings is not the same measurement. The naming requirement matters more than the number, because it lets the threshold be argued with rather than trusted. |
| ~~**D15**~~ | ~~Write the three `NotYetWritten` fixtures?~~ **Resolved: yes, and two are not coverage work.** | `declared-size-mismatch` and `manifest-mismatch` are the malformed-input classes. Unwitnessed, the classifier's behaviour on hostile input is unverified — which ADR-F026 and the no-panics-on-malformed-input standard both require, so this is a correctness gap wearing a coverage gap's clothes. `extra-field` is genuine coverage (Unix perms, UT timestamps, Zip64 are common in the wild). None expands the *judgement* surface: all three witness classes already frozen under ADR-F041, so the taxonomy does not move. |
...
| ~~**D12**~~ | ~~Producer-diversity bar for Tier B?~~ **Resolved: coverage, not count.** Floor set at **≥8 distinct un-normalized producers, with no single producer exceeding 40% of that population.** | A bare count is the wrong instrument for a coverage rule: twenty producers where one holds 95% is a worse corpus than six held evenly, and a count-only floor cannot tell them apart. Set by what makes the result meaningful, not by what the library happens to contain — if it cannot clear the bar, `Inconclusive` is the correct and honest output. Note the gate's purpose has shifted: it no longer adjudicates rbook, it validates `futhark-epub` once built. |
